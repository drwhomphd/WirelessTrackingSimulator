...
def load_analysis_input(inputFile):
    ...
    for line in input.readlines():
        ...
        if NODE_TYPE == 0:
            innerHits = elements[7]
            outerHits = elements[8]
        elif NODE_TYPE == 1:
            innerHits = elements[11]
            outerHits = elements[12]
        else:
            innerHits = elements[7] + elements[11]
            outerHits = elements[8] + elements[12]
        # End if
...
def calculate_agent_positions(agentRouterDetections, routerDatabase, outerSquare, innerSquare):
    ...
    for node in agentRouterDetections:
        ...
        for round in agentRouterDetections[node]:
            ...
            #locationDatabase[node] = {round: [innerDetectSquare, outerDetectSquare]}
        # End for
...
def write_agent_positions(outputFile, agentPositionDatabase):
    ...
    for node in agentPositionDatabase:
        ...
        for round in agentPositionDatabase[node]:
            ...
            #output.writelines("%s,%s,%f~%f|%f~%f|%f~%f|%f~%f,%f~%f|%f~%f|%f~%f|%f~%f,%f~%f|%f~%f|%f~%f|%f~%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n" % (node, round, iUL[0], iUL[1], iUR[0], iUR[1], iLR[0], iLR[1], iLL[0], iLL[1], oUL[0], oUL[1], oUR[0], oUR[1], oLR[0], oLR[1], oLL[0], oLL[1], bUL[0], bUL[1], bUR[0], bUR[1], bLR[0], bLR[1], bLL[0], bLL[1], innerArea, innerWidth, innerHeight, outerArea, outerWidth, outerHeight, bothArea, bothWidth, bothHeight))
