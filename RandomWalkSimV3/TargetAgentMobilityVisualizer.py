...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    ...
    for personID in personTable.keys():
        ...
        # Check to see if this person is the one we're supposed to detect
        # If they aren't, skip...
        if personID != PERSON_TO_DETECT:
            continue
        # End if
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) < 4):
        print("DataVisualizer.py [Input Router File Name] [Output File Name] [Person to Detect] [Cell detect radius*]")
        sys.exit(1)
    else:
        personFile = sys.argv[1]
        outFile = sys.argv[2]
        if len(sys.argv) == 4:
            PERSON_TO_DETECT = sys.argv[3]
    ...
    print("Person to Detect: %s" % (PERSON_TO_DETECT))
    print("Cell Detection Radius %f Meters" % (DETECT_RADIUS))
