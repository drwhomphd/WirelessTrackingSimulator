...
echo "average.detected.walk.length,average.undetected.walk.length,std.dev.detected,std.dev.undetected,city,pop,time,perc,radius" > POPULATION_average_walk_length_stats-PercentDetected${det}-Outer.csv
echo "average.detected.walk.length,average.undetected.walk.length,std.dev.detected,std.dev.undetected,city,pop,time,perc,radius" > POPULATION_average_walk_length_stats-PercentDetected${det}-Inner.csv
city="Chicago-"
...
#city,pop,time,perc,radius
            ...
            echo $fileDir
            ./PopulationAvgDetectedWalkLengthForPercDetects.py ${fileDir}/udel-statdata.out-cell  $det 0 $city $pop $time $perc $radius >> POPULATION_average_walk_length_stats-PercentDetected${det}-Inner.csv
            ./PopulationAvgDetectedWalkLengthForPercDetects.py ${fileDir}/udel-statdata.out-cell $det 1 $city $pop $time $perc $radius >> POPULATION_average_walk_length_stats-PercentDetected${det}-Outer.csv
