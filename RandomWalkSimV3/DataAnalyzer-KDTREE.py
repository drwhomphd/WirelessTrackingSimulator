...
def child_process(pipeout, startOfRange, endOfRange, databaseFile):
    ...
    for r in range(startOfRange, endOfRange+1):
        ...
        # round
        # Check and see if we want routers and other nodes or not
        if -1 in nodeListDict:
            # We have routers
            kdtreeDict[r] = KdTree(nodeListDict[r] + nodeListDict[-1])
        else:
            # We don't have routers
            kdtreeDict[r] = KdTree(nodeListDict[r])
        # End if
    ...
    for key in lookupDictionary.iterkeys():
        ...
        can_detect = lookupDictionary[key][6]
        ...
        #       inner cell hits, outer cell hits, inner cell hit ids, outer cell hit ids, can detect?
        ...
        # Add in Can Detect at the end
        outString.write(",%s" % can_detect)
...
if __name__ == '__main__':
    ...
    rows = list(curs.execute("SELECT MAX(round), COUNT(obj_id) FROM tree_data WHERE obj_type != 0"))
